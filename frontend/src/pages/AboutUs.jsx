...
const GoalCard = ({ icon, title, description, delay }) => {
  ...
  return (
    ...
    >
      <div className="w-14 h-14 bg-[#1A0A62]/5 rounded-xl flex items-center justify-center mb-6 text-[#1A0A62]">
        <span className="material-symbols-outlined text-3xl">{icon}</span>
      </div>
      <h3 className="text-xl font-bold mb-3 text-[#1A0A62]">{title}</h3>
...
const ContactInfo = ({ icon, label, value, delay, isPrimary = false }) => {
  ...
  return (
    <div 
      ...
      style={{ transitionDelay: `${delay}ms` }}
    >
      <div className={`p-4 rounded-full ${isPrimary ? 'bg-[#F24E1E]/10 text-[#F24E1E]' : 'bg-[#1A0A62]/5 text-[#1A0A62]'}`}>
        <span className="material-symbols-outlined">{icon}</span>
      </div>
      <div>
        <p className="text-xs font-bold uppercase tracking-wider text-slate-500 mb-1">{label}</p>
        <p className={`font-bold ${isPrimary ? 'text-xl text-[#1A0A62]' : 'text-lg text-slate-700'}`}>{value}</p>
      </div>
    </div>
  );
};
  ...
  return (
    <Layout>
      <main className="font-display overflow-x-hidden">
        {/* Hero Section */}
        <section className="relative py-24 bg-[#0D0531] text-white">
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            <div className="flex flex-col lg:flex-row items-center gap-16">
              ...
              >
                <span className="inline-block px-4 py-1.5 rounded-full bg-[#F24E1E]/20 text-[#F24E1E] font-bold text-xs uppercase tracking-widest mb-6">Expert Guidance Since 2010</span>
                <h1 className="text-4xl md:text-6xl font-black leading-tight mb-8">
                  About <span className="text-[#F24E1E]">Apex</span> Counselling Group
                </h1>
                ...
                <div className="flex flex-wrap justify-center lg:justify-start gap-4">
                  <a className="bg-[#F24E1E] text-white px-8 py-4 rounded-xl font-bold hover:bg-orange-600 transition-all flex items-center gap-2 shadow-xl shadow-[#F24E1E]/20" href="https://apexclasses.org" target="_blank" rel="noopener noreferrer">
              ...
              <div className="flex-1 w-full max-w-xl group">
                <div className="rounded-3xl overflow-hidden shadow-2xl border-8 border-white/5 relative">
                  ...
                  />
                  <div className="absolute inset-0 bg-gradient-to-t from-[#0D0531]/40 to-transparent"></div>
                </div>
              </div>
            </div>
          </div>
        ...
        <section className="py-24 bg-slate-50">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center mb-20" ref={goalsRef}>
              <h2 className={`text-4xl font-bold text-[#1A0A62] mb-4 transition-all duration-700 ${goalsVisible ? 'opacity-100' : 'opacity-0 translate-y-4'}`}>Our Core Goals</h2>
              <div className={`w-20 h-1.5 bg-[#F24E1E] mx-auto rounded-full transition-all duration-700 delay-300 ${goalsVisible ? 'w-20' : 'w-0'}`}></div>
            </div>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex flex-col lg:flex-row gap-20">
              <div className="flex-1">
                <h2 className="text-4xl font-black text-[#1A0A62] mb-6">Get in Touch</h2>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <button className="bg-[#1A0A62] text-white flex items-center justify-center gap-3 py-5 rounded-2xl font-bold hover:bg-[#0D0531] transition-all shadow-xl shadow-[#1A0A62]/20">
              ...
              <div className="flex-1">
                <div className="h-full min-h-[500px] rounded-[2.5rem] overflow-hidden shadow-2xl border-4 border-slate-100 relative group">
                  ...
                  />
                  <div className="absolute inset-0 bg-navy/10 group-hover:bg-transparent transition-all duration-700"></div>
                  <div className="absolute bottom-8 left-8 right-8 bg-white p-8 rounded-3xl shadow-2xl transform translate-y-4 group-hover:translate-y-0 transition-all duration-700">
                    <div className="flex items-center justify-between">
                      <div>
                        <h4 className="font-extrabold text-[#1A0A62] text-xl mb-1">Regional Office</h4>
                        <p className="text-sm text-slate-500 font-medium">Maharashtra Regional Headquarters</p>
                      </div>
                      <div className="bg-[#F24E1E] p-4 rounded-2xl text-white shadow-lg shadow-[#F24E1E]/30">
